...
//!
//! # Features
//! GRUB Multiboot2 / UEFI / DTB 등 이질적 펌웨어 핸드오프를
...
/// mb2 어댑터가 채우지 않는 필드는 0 (미제공) 초기값으로 유지되며 dtb, kaslr,
/// framebuffer, rsdp 소비 시점에 실사용 배선된다.
pub struct BootInfo {
    /// 물리 메모리 맵 (사용 가능 영역 확정)
    pub memory_map: MemoryMap,
    /// KASLR 직접 선형 매핑 오프셋 (0 = 미제공)
    ...
    #[allow(dead_code)]
    pub rsdp_ptr: u64,
    /// Device Tree Blob 물리 포인터 (0 = 미제공, aarch64)
    #[allow(dead_code)]
    pub dtb_ptr: u64,
