//! Air-gap 이중 게이트 표면과 감사 조회 syscall, 2 층 self-check 를 제공하는 모듈입니다.
//!
//! # Features
//! tls-external feature 게이트 안에서 NETWORK_ATTACH_CAP one-shot mint 와 take, handle_attach Network arm 을 연계합니다.
//! 양 프로필 공통으로 AUDIT_READ_CAP one-shot mint 와 take, sys_hsm_status 456 옥텟 atomic 응답을 제공합니다.
//! 양 프로필 공통 gap_self_check 는 boot-time fail-stop 으로 closed 빌드의 심볼 부재 invariant 를 검증합니다.
//!
//! # 책임 경계
//! - 본 모듈은 air-gap 정책 표면만 제공하고, AUDIT_RING 자체는 hsm_attest 모듈이 책임집니다 (audit_enqueue 호출자)
//! - take_*_cap 의 SMAP 윈도우는 단일 stac/clac 로 처리합니다
//! - cap 비교는 모두 CtEqOps eq 로 하며 early-return 을 금지합니다
//! - state 전이는 SMAP clac 이후에만 수행하여 cap take race 를 회피합니다
//! - handle_status 본 호출은 AUDIT_RING 에 기록하지 않아 audit-of-audit 를 회피합니다
...
//
// NetCapState FSM Provisioned 0 Taken 1 단방향
//

/// one-shot cap take FSM 상태
///
/// 부팅 시 BSS default 가 Provisioned, take 호출 시 Taken 으로 단방향 전이합니다.
...
//
// NETWORK_SYM_PRESENT 컴파일 시점 cfg const
//
...
//
// AUDIT_READ_CAP 양 프로필 공통 BSS singleton
//
...
//
// NETWORK_ATTACH_CAP tls-external 게이트 BSS singleton
//
...
//
// GAP_STATUS_* ABI 잠금 sys_hsm_status 456 옥텟 layout
//
// header 8 + StatusEntry 64 (= 8 * 8) + EnrollEvent 384 (= 32 * 12) = 456 옥텟
// sibling test 의 byte-exact 일치 잠금 대상
...
/// `bus_kind` BusKind octet
/// `attest_result` verify_result_code
/// `_pad` align fill 항상 0
/// `pk_hash_prefix` BLAKE3 prefix 4 옥텟
#[repr(C)]
...
//
// init_audit_read_cap 부팅 시 1 회 호출
//
...
pub unsafe fn init_audit_read_cap() {
    // gen_token_u64 2 회 호출, 첫 번째만 token 사용 두 번째는 DRBG entropy hop
    ...
    // rights 와 slot 은 invalid 잠금, Ring 3 측 검증은 token only
...
//
// init_network_cap 부팅 시 1 회 호출 tls-external 한정
//
...
//
// gap_self_check 2 층 자체 점검
//

/// 부팅 마지막 단계 air-gap 표면 무결성 자체 점검 fail-stop
///
/// # Safety
/// kernel_main 의 모든 init_* 호출과 syscall dispatcher 등록 완료 이후 단일 호출
...
pub unsafe fn gap_self_check() {
    // tls-external 빌드 한정 NETWORK_ATTACH_CAP sanity 점검
    #[cfg(feature = "tls-external")]
    {
    ...
    // closed 빌드 한정 NETWORK_SYM_PRESENT false 컴파일 시점 fold
    #[cfg(not(feature = "tls-external"))]
    {
        const _: () = assert!(!NETWORK_SYM_PRESENT);
    }
    // 양 프로필 공통 AUDIT_READ_CAP sanity 점검
...
// syscall handler 본문
//
// take_*_cap one-shot 16 B out, FSM Provisioned 에서 Taken 전이
// handle_status atomic 456 B out, cap-fail 만 AUDIT 기록

/// sys_network_cap_take Ring 3 NETWORK_ATTACH 캡 인도 syscall handler
///
/// tls-external 빌드 한정 closed 빌드 cfg-out
/// out_ptr rdi 16 옥텟 HsmCapability 응답, first-caller-wins 이며 after-take 는 Denied 로 수렴
#[cfg(feature = "tls-external")]
pub fn take_network_cap(ctx: &mut SyscallContext) -> u64 {
    // 레지스터 스냅샷, 단일 out 인자만
    let out_ptr = ctx.arg0;

    // dual-range 주소 검증, BadAddress 도 Denied 로 수렴
    ...
    // FSM 상태 검증, Provisioned 만 통과 Taken 재호출은 Denied 로 수렴
...
/// sys_audit_cap_take Ring 3 AUDIT_READ 캡 인도 syscall handler
///
/// 양 프로필 공통 out_ptr rdi 16 옥텟 응답 first-caller-wins after-take Denied
/// take_network_cap 클론, AUDIT_* slot 0xFD bus_kind Software 로 치환
pub fn take_audit_read_cap(ctx: &mut SyscallContext) -> u64 {
    ...
    if current_state != NetCapState::Provisioned {
        // result 2, slot 0xFD AUDIT_READ 식별자, bus_kind Software 0 도용
...
/// sys_hsm_status atomic 456 옥텟 응답 syscall handler
///
/// 양 프로필 공통 AUDIT_READ cap 보유자만 진입
/// out_ptr rdi out_len rsi caller_cap_token rdx ABI 잠금
/// 호출 자체는 AUDIT_RING 미기록, audit-of-audit 회피, cap-fail 만 result 2 기록
pub fn handle_status(ctx: &mut SyscallContext) -> u64 {
    ...
    // out_len 정합성 검사, BufferTooSmall 은 Denied 로 수렴 AUDIT 미기록
    // SMAP 윈도우 진입 이전이고 AUDIT 미기록, caller-side 정보만 사용하므로 정수 < 비교로 충분
    if out_len < GAP_STATUS_LEN as u64 {
        return SyscallError::Denied.as_rax();
    }

    // cap ct_eq, early-return 금지하고 CtEqOps unwrap_u8 로 한 번에 결정
    let cap_eq = unsafe {
        let stored = (&raw const AUDIT_READ_CAP).read();
        use constant_time::traits::CtEqOps;
        CtEqOps::ct_eq(&stored.token, &caller_cap_token).unwrap_u8() == 1
    };
    if !cap_eq {
        // result 2, slot 0xFF sys_hsm_status cap-fail 식별자, bus_kind Software 0
        audit_enqueue(0xFF, 2, BusKind::Software as u8, [0u8; 4]);
        return SyscallError::Denied.as_rax();
    }

    // dual-range out_ptr length-aware 검증, BadAddress 는 Denied 와 분리
    if !is_user_address(out_ptr) || !is_user_address(out_ptr.saturating_add(out_len)) {
        return SyscallError::BadAddress.as_rax();
    }

    // stack staging 456 옥텟
    let mut staging = [0u8; GAP_STATUS_LEN];

    // 데이터 수집, SMAP 윈도우 밖에서 수행
    // (a) StatusEntry 8 슬롯 채움, _reserved layout mirror
    ...
    for (i, slot_info) in slot_infos.iter().enumerate() {
        ...
        // HsmSlotInfo 메모리 layout 자체가 그 매핑 그대로 (slot=offset0 state=offset1 _reserved[0..6]=offset2..8)
        // 단 StatusEntry 는 (slot_idx=0 bus_kind=1 attest_result=2 _pad=3 pk_hash[4..8])
        // 이 어긋남 때문에 raw bytes 를 staging[off..off+8] 로 통째 copy 하면 'state' octet 이 'bus_kind' 자리에 가지 못함
        // 따라서 아래처럼 field-by-field 직렬화 적용
        let info_bytes: [u8; 8] = unsafe {
            core::mem::transmute::<HsmSlotInfo, [u8; 8]>(*slot_info)
        };
        // info_bytes 레이아웃 [slot, state, _reserved[0..6]]
        // _reserved[0] = bus_kind, _reserved[1] = verify_result_code
        // _reserved[2..6] = pk_hash_prefix
    ...
    // (d) EnrollEvent written 개 raw 12 옥텟 직렬화, bus.rs wire mirror byte-exact
    for (i, ev) in events_local.iter().enumerate().take(audit_written) {
    ...
    // SMAP 단일 윈도우, stac copy clac 456 옥텟
    ...
    // stack-local zeroize, staging 과 events_local 양면
    staging.zeroize();
