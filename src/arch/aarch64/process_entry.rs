...
//!
//! # Features
//! TTBR0_EL1 적재 -> TLB flush -> ELR_EL1/SP_EL0/SPSR_EL1 세팅 -> `eret` 를 단일
...
//! 합류합니다(ARM-09).
//!
//! # DIVERGENCE (10-PATTERNS L444-448)
//!   - `mov cr3` -> `msr ttbr0_el1` + isb + `tlbi vmalle1` + `dsb ish` (TLB flush 명시).
//!   - GS-base 전환 대응 없음 (EL0 스택은 SP_EL0 별도 레지스터).
//!   - iretq frame(SS/RSP/RFLAGS/CS/RIP) -> `msr elr_el1`(RIP) + `msr sp_el0`(RSP)
//!     + `msr spsr_el1`(EL0t 모드) + `eret`. x86 유저 세그먼트 셀렉터 개념 부재
//!     -- SPSR_EL1.M[4:0]=0 이 EL0 모드를 선택함.
//!
//! # Authors
//! Q. T. Felix
...
pub unsafe fn enter_user(ttbr0: u64, entry: u64, stack: u64) -> ! {
    // SAFETY: 아래 asm 블록은 단일 atomic 시퀀스로 ttbr0 적재 -> TLB flush ->
    //         elr_el1/sp_el0/spsr_el1 세팅 -> eret 순으로 실행. 사이에 어떤 high-level
